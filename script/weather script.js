...
document
  ...
  .forEach((button) =>
    button.addEventListener('click', (e) =>
      getWeatherData(e.target.innerText).then(
        renderWeatherModal,
        showErrorModal
      )
    )
...
function renderWeatherModal(weatherData) {
  ...
  let feelsLikeTemperatureElement = document.getElementById(
  ...
  iconElement.setAttribute(
    'src',
    `../weather-icons/${weatherData.weather[0].icon}.png`
  )
  descriptionElement.innerText = `${weatherData.weather[0].description}`
  temperatureElement.innerText = `Temperature: ${formatTemperatureData(
    weatherData,
    'temp'
  )}`
